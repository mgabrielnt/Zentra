        ...
        {mobileMenuOpen && (
          <div className="lg:hidden mt-2 mx-6 bg-white/10 backdrop-blur-lg rounded-3xl shadow-2xl overflow-hidden border border-white/20">
              ...
              {navItems.map((item, index) => (
                <li key={index}>
                  <Link 
                    ...
                    className={`block px-6 py-3 font-bold text-sm transition-all ${
                      pathname === item.href 
            ...
            <div className="flex items-center gap-3 px-6 pb-4 pt-2">
              <button 
                onClick={() => setLanguage('en')} 
                className={`relative flex-1 py-3 rounded-full text-sm font-bold tracking-wide transition-all duration-500 ease-out overflow-hidden ${
                  language === 'en' 
                    ? 'bg-linear-to-br from-white via-white to-gray-50 text-gray-900 shadow-xl scale-105' 

...
/** ---- Util: palette texture (reusable untuk init & update) ---- */
...
export default function LiquidEther({
  ...
  /** NEW: agar tetap mengikuti mouse walau tertutup elemen lain */
  globalPointer = true,
}) {
  ...
  const paletteRef = useRef(null); // simpan texture palette untuk dispose saat update

  /** ========== SHADERS & CLASSES (identik dengan punyamu) ========== */
  const bgVec4 = useRef(new THREE.Vector4(0, 0, 0, 0)).current;

  class CommonClass {
    constructor() {
      this.width = 0; this.height = 0; this.aspect = 1; this.pixelRatio = 1;
      this.isMobile = false; this.breakpoint = 768; this.fboWidth = null; this.fboHeight = null;
  ...
  const Common = useRef(new CommonClass()).current;
  ...
  class MouseClass {
    constructor() {
      ...
      this._onPointerMoveWin = this.onDocumentMouseMove.bind(this);
      this._onPointerLeaveWin = this.onMouseLeave.bind(this);
      ...
      this.globalPointer = true;
    }
    init(container, opts = {}) {
      this.container = container;
      this.globalPointer = opts.globalPointer ?? true;
      ...
      if (this.globalPointer) {
        window.addEventListener('pointermove', this._onPointerMoveWin, { passive: true, capture: true });
        window.addEventListener('pointerleave', this._onPointerLeaveWin, { passive: true, capture: true });
      } else {
    ...
    dispose() {
      ...
      if (this.globalPointer) {
        window.removeEventListener('pointermove', this._onPointerMoveWin, { capture: true });
        window.removeEventListener('pointerleave', this._onPointerLeaveWin, { capture: true });
      }
    }
    setCoords(x, y) {
      if (!this.container) return;
  ...
  const Mouse = useRef(new MouseClass()).current;

  /** ---- shader strings (sama seperti punyamu) ---- */
  const face_vert = `
  ...
  class Output {
    constructor(paletteTex) { this.init(paletteTex); }
    init(paletteTex) {
  ...
  class WebGLManager {
    constructor(props) {
      ...
      Mouse.init(props.$wrapper, { globalPointer: props.globalPointer });
      ...
      this.autoDriver = new (class AutoDriver {
      ...
      this._onVisibility = () => { const hidden = document.hidden; if (hidden) this.pause(); else if (isVisibleRef.current) this.start(); };
      document.addEventListener('visibilitychange', this._onVisibility);
      this.running = false;
    }
    init() {
      this.props.$wrapper.prepend(Common.renderer.domElement);
      this.output = new Output(this.props.paletteTex);
    }
  ...
  useEffect(() => {
    ...
    // Visibility/pause saat tidak terlihat
    ...
    return () => {
      ...
      if (paletteRef.current) { paletteRef.current.dispose?.(); paletteRef.current = null; }
      webglRef.current = null;
    };
    // ⛔️ deps dikosongkan agar ukuran array tetap stabil (tidak berubah)
    // Semua perubahan props ditangani di EFFECT #2 (di bawah).
  }, []); // <-- stable, tidak berubah ukuran

  /** ====================== EFFECT #2: UPDATE PROP RUNTIME ====================== */
  // Satu dependency saja: signature string → ukuran array SELALU 1 (permanent fix)
  const depSignature = useMemo(
    () =>
      JSON.stringify({
        BFECC, cursorSize, dt, isBounce, isViscous,
        iterationsPoisson, iterationsViscous, mouseForce, resolution, viscous,
        colors, autoDemo, autoSpeed, autoIntensity, takeoverDuration, autoResumeDelay, autoRampDuration, globalPointer,
      }),
    [BFECC, cursorSize, dt, isBounce, isViscous, iterationsPoisson, iterationsViscous, mouseForce, resolution, viscous, colors, autoDemo, autoSpeed, autoIntensity, takeoverDuration, autoResumeDelay, autoRampDuration, globalPointer]
  );
  ...
  useEffect(() => {
    ...
    // update palette jika colors berubah
    if (webgl.output?.output?.material?.uniforms?.palette) {
      const newTex = makePaletteTexture(colors);
      const u = webgl.output.output.material.uniforms;
      const oldTex = u.palette.value;
      u.palette.value = newTex;
      if (paletteRef.current && paletteRef.current !== newTex) {
        paletteRef.current.dispose?.();
      }
      paletteRef.current = newTex;
